...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Shader "Hidden/Fronkon Games/Glitches/JPEG URP"
{
  Properties
  {
  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      half4 GlitchesFrag(const VertexOutput input) : SV_Target
      {
        ...
        UNITY_BRANCH
        if (SAMPLE_TEXTURE2D_X(_NoiseTex, sampler_LinearRepeat, uv_noise).r < blocks ||
            SAMPLE_TEXTURE2D_X(_NoiseTex, sampler_LinearRepeat, float2(uv_noise.y, 0.0)).g < lines)
        {
        ...
        UNITY_BRANCH
        if (SAMPLE_TEXTURE2D_X(_NoiseTex, sampler_LinearRepeat, uv_noise).g < blocks)
          pixel.rgb = lerp(pixel.rgb, ColorBlend(_BlockBlend, pixel.rgb, _BlockTint.rgb) , _BlockTint.a);
        
        UNITY_BRANCH
        if (SAMPLE_TEXTURE2D_X(_NoiseTex, sampler_LinearRepeat, float2(uv_noise.y, 0.0)).b * 3.5 < lines)
          pixel.rgb = lerp(pixel.rgb, ColorBlend(_LineBlend, pixel.rgb, _LineTint.rgb) , _LineTint.a);

        UNITY_BRANCH
        if (SAMPLE_TEXTURE2D_X(_NoiseTex, sampler_LinearRepeat, uv_noise).g * 1.5 < blocks ||
            SAMPLE_TEXTURE2D_X(_NoiseTex, sampler_LinearRepeat, float2(uv_noise.y, 0.0)).g * 2.5 < lines)
        {
