...
namespace VolFx
{
    ...
    public class GlitchPass : VolFx.Pass
    {
		...
		public override void Init()
		{
			...
			_shotTex = new RenderTarget().Allocate(new RenderTexture(Screen.width, Screen.height, GraphicsFormat.R8G8B8A8_UNorm, GraphicsFormat.None), $"{name}_shot_rt");
			_lockTex = new RenderTarget().Allocate(new RenderTexture(Screen.width, Screen.height, GraphicsFormat.R8G8B8A8_UNorm, GraphicsFormat.None), $"{name}_lock_rt");
